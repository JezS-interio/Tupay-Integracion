...
/**
 * Upload smartphone product images to Cloudflare R2
 * Reads from scripts/phone_products/ directory
 */
...
async function main() {
  console.log("📱 Upload Smartphone Products to R2\n");
  console.log("=" .repeat(60));

  const imagesDir = path.join(__dirname, "phone_products");

  if (!fs.existsSync(imagesDir)) {
    console.error(`❌ Error: Directory not found: ${imagesDir}`);
    console.error("Run prepare-phone-products.py first!");
    process.exit(1);
  }
  ...
  console.log(`📁 Found ${files.length} smartphone images to upload\n`);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const r2Key = `products/phones/${filename}`;
  ...
  console.log("Go to: http://localhost:3000/admin/import-phone-products");
  console.log("to import these smartphones to Firestore");
}
